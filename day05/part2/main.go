...
func main() {
	...
	for ss, s := range seeds {
		fmt.Printf("Processing seed set %d:\n", ss)
		for offset := 0; offset < s[1]; offset++ {
			x := findInMapping(s[0]+offset, seedToSoil)
...
func parseInput(fn string) {
	...
	for scanner.Scan() {
		...
		if seedSoilMatch != nil { //has seed-soil mappings
